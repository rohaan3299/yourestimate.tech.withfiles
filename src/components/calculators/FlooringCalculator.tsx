...
export default function FlooringCalculator() {
  ...
  const calculateFlooring = () => {
    ...
    if (rl && rw && tl && tw) {
      ...
      const tileAreaMeters = tileArea * 0.092903; // Converting to sq meters
      const tilesNeeded = Math.ceil(roomArea / tileArea);
      const tilesNeededMeters = Math.ceil(roomArea * 0.092903 / tileAreaMeters);
      
      setResult({
        tiles: tilesNeeded,
        tilesInMeters: tilesNeededMeters,
        ...
        <div className="space-y-4">
          ...
          {result && (
            <div className="mt-6 p-4 bg-blue-50 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-gray-600">Tiles Needed (sq ft):</p>
                  <p className="text-lg font-semibold text-gray-800">{result.tiles} pieces</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Tiles Needed (sq m):</p>
                  <p className="text-lg font-semibold text-gray-800">{result.tilesInMeters} pieces</p>
                </div>
                <div>
